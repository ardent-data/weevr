...
class TestWriteTargetMerge:
    ...
    def test_merge_soft_delete_default_value_seeds_schema(
        self, spark: SparkSession, tmp_delta_path, simple_df
    ) -> None:
        """First merge with soft_delete_default_value seeds the column with that value."""
        ...
        result = spark.read.format("delta").load(path)
        assert "is_deleted" in result.columns
        # First write seeds with soft_delete_default_value, not null
        assert all(r["is_deleted"] is False for r in result.collect())

    def test_merge_soft_delete_default_value_on_match_and_insert(
        self, spark: SparkSession, tmp_delta_path
    ) -> None:
        """soft_delete_default_value is written to matched/inserted rows;
        ...
        result = spark.read.format("delta").load(path)
        rows = {r["id"]: r for r in result.collect()}
        # id=1 re-appeared: soft_delete_default_value (False) should be written
        ...
        # id=4 was inserted: soft_delete_default_value (False) should be written
        assert rows[4]["is_deleted"] is False

    def test_merge_soft_delete_default_value_none_preserves_null(
        self, spark: SparkSession, tmp_delta_path
    ) -> None:
        """soft_delete_default_value=None (default) keeps existing null behaviour
        on update/insert."""
        from pyspark.sql.types import BooleanType, LongType, StringType, StructField, StructType
        ...
        write_config = WriteConfig(
            ...
            on_no_match_source="soft_delete",
            soft_delete_column="is_deleted",
            # soft_delete_default_value omitted → None
        )
        write_target(spark, incoming, target, write_config, path)
        result = spark.read.format("delta").load(path)
        rows = {r["id"]: r for r in result.collect()}
