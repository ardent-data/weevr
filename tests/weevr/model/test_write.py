...
class TestWriteConfig:
    ...
    def test_soft_delete_default_value_defaults_to_none(self):
        """soft_delete_default_value is None by default."""
        w = WriteConfig()
        assert w.soft_delete_default_value is None

    def test_soft_delete_default_value_false_round_trip(self):
        """soft_delete_default_value=False round-trips correctly."""
        w = WriteConfig(
            mode="merge",
            match_keys=["id"],
        ...
        assert restored == w
        assert restored.soft_delete_default_value is False

    def test_soft_delete_default_value_true_round_trip(self):
        """soft_delete_default_value=True round-trips correctly."""
        w = WriteConfig(
            mode="merge",
            match_keys=["id"],
            on_no_match_source="soft_delete",
            soft_delete_column="is_deleted",
            soft_delete_default_value=True,
        )
        restored = WriteConfig.model_validate(w.model_dump())
        assert restored == w
        assert restored.soft_delete_default_value is True
