        ...
        if mode == "overwrite" or not target_exists:
            ...
            if (
                ...
                df = df.withColumn(
                    ...
                    F.lit(write_config.soft_delete_default_value).cast("boolean"),
                )
...
def _execute_merge(
    ...
    # Matched rows and new inserts receive soft_delete_default_value (null when not set,
...
def execute_cdc_merge(
    ...
    try:
        ...
        if update_val:
            ...
            # When soft_delete_default_value is set, reset the flag on re-appearing rows.
            if (
                write_config.soft_delete_column
                and write_config.soft_delete_default_value is not None
            ):
                update_set[write_config.soft_delete_column] = F.lit(
                    write_config.soft_delete_default_value
                )
        ...
        if insert_val:
            ...
            # When soft_delete_default_value is set, seed the flag on new inserts.
            if (
                write_config.soft_delete_column
                and write_config.soft_delete_default_value is not None
            ):
